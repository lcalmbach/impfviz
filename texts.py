text_dashboard = """Es werden nur Personen mit Wohnsitz im Kanton Basel-Stadt berücksichtigt. Annahmen: Die Wirksamkeit 
der Impfung oder einer Infektion dauert {}. Für detaillierte Angaben zur Methodik wähle das Menu `Methodik` im Seitenfenster aus.<br><br>"""
...
Impf-Dashboards haben zum Ziel, Auskunft über den Impfstatus der Bevölkerung zu geben. Klassische Impfdashboards zeigen zu diesem Zweck die kumulierte Anzahl der partiell, vollständig- und Auffrisch-Geimpften. Dabei wird nicht berücksichtigt, dass ein beträchtlicher Teil der Bevölkerung durch Infektion zusätzlich geimpft wird. Ebenfalls nicht berücksicht wird die Tatsache, dass sich der Impfschutz mit der Zeit reduziert. Das Ignorieren dieser beiden Parameter kann damit begründet werden, dass die genauen Daten dazu in der Regel nicht vorliegen: die Dauer der Wirksamkeit der Impfung ist von Person zu Person unterschiedlich und es kann auch keine scharfe Grenze (vorher geschützt, nachher nicht mehr) gezogen werden. Bei den infizierten Personen ist unbekannt, ob diese vorher ungeschützt oder bereits einen Impfstatus hatten. Für beide Faktoren kann man jedoch Annahmen treffen. Im Vorliegenden Modell wurde angenommen, dass eine Infektion bei nichtgeimpften Personen fünf Mal wahrscheinlicher ist als bei den ungeimpften oder bei Personen mit abgelaufenen Impfschutz. Ist zum Beispiel in der Bevölkerung das Verhältnis geimpft/ungeimpft am Tag X 1:1 und es wurden an diesem Tag 100 Personen infiziert, so wurden im Modell für diesen Tag 2 bereits geimpfte, und 8 ungeimpfte Personen aus dem Bevölkerungsmodell in den Status *geimpft durch Infektion* versetzt. In der vorliegenden App kann aus drei Szenarien ausgewählt werden: Impfschutz für 1 Jahr bzw. 6 oder 8 Monate. Für jeden Tag versetzt das Modell alle Personen in den Status *Impfschutz abgelaufen*, deren Datum "Letzte Impfung oder Infektion" länger als die Periode des Impfschutz im aktuellen Szenario zurückliegt. 
